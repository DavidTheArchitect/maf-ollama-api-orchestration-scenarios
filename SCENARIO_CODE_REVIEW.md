...
code that runs them, across both packages (`release_room`, `review_bot`), the
...
| A2 | `_SESSION_TURNS` in `review_bot/server.py` grew without bound: every session and every turn was retained forever. | Bug (leak) | Bounded: `_MAX_SESSIONS = 64` with oldest-first eviction and `_MAX_TURNS_PER_SESSION = 40` per-session trim, via `_session_history` / `_record_turns`. Covered by tests. |
...
| A4 | `release_room` buried runtime output extraction inside `notebook_helpers.py` while `review_bot` had a dedicated `output_formatting.py` — same logic, inconsistent structure. | Structure | `release_room/output_formatting.py` created; `notebook_helpers` re-exports so existing imports keep working; `workflows.py` imports the canonical module. Cross-*package* duplication is retained deliberately: the two samples are standalone teaching artifacts. |
