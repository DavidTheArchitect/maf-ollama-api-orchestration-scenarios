"""Group chat policy exception board job scenario (MCP tools) for the Invocations API sample."""
...
SCENARIO = ScenarioSpec(
    ...
    pattern="group-chat",
    title="Group Chat Policy Exception Board Job",
    learning_goal="Learn how a group chat board job debates a policy exception, with members grounding risk, business need, and compliance in local MCP tools before a chair records the recommendation.",
    when_to_use="Use Invocations plus group chat orchestration when an internal job needs a documented board decision backed by tool-grounded facts.",
    sample_task="Run a policy exception board job for POLICY-EX-77 (temporary data residency waiver) and return an approved recommendation with a compensating control and expiry.",
    agents=(
        AgentSpec(
            "RiskAssessorAgent",
