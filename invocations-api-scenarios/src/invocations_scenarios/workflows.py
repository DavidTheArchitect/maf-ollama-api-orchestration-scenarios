...
(see :mod:`invocations_scenarios.executors`) with agent nodes wrapped in
...
from .models import InvocationRequest, InvocationResponse, build_invocation_prompt
...
async def run_scenario_sample(
    ...
    **config_overrides: Any,
    ...
    request = InvocationRequest(
        ...
        subject="scenario sample run",
    )
    response = await run_invocation(request, max_tokens=tokens, **config_overrides)
    return response.summary


def build_invocation_workflow(
    scenario: ScenarioSpec | str | None = None,
    *,
    model: str | None = None,
...
def make_group_chat_termination(
    ...
    Cycle boundaries are inferred from ``len(assistant) % participant_count``,
    which assumes each participant emits exactly one assistant message per
    round. If a seat emits zero or several (for example a remote A2A peer in
    Scenario 17 that errors or replies twice), the count drifts, phrase-based
    early termination may not fire on the intended turn, and the run falls back
    to the ``max_cycles`` cap. Bounded either way — this is a teaching
    simplification, not a hard guarantee.
    """
...
async def run_invocation(
    ...
    *,
    ...
    prompt = build_invocation_prompt(request, previous_turns)
    workflow = build_invocation_workflow(
        scenario,
        model=model,
...
def _recommendations_from_text(text: str) -> list[str]:
    ...
    return recommendations or ["Inspect the summary and decide whether follow-up specialist work is needed."]
