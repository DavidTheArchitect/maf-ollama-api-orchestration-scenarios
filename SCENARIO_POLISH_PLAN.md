...
## Workstream B — Logic double-check (all 20 scenarios, both packages)
...
- All 40 notebooks (regenerated, not hand-edited)
- Scenario modules with logic/depth/docs fixes: `responses-api-scenarios/src/release_room/scenarios/*.py` + mirrors in `invocations-api-scenarios/src/review_bot/scenarios/*.py`
...
- Regenerate all 40 notebooks via the generator; `python -m compileall` both packages + scripts.
